        ...
        <div className={`relative h-screen w-full overflow-hidden font-sans select-none ${theme === 'high-contrast' ? 'bg-black text-white' : 'bg-neutral-950 text-white'}`}>
            ...
            >
                <div className="flex items-center gap-6 bg-neutral-900/80 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-2xl shadow-2xl">
                    <button onClick={() => { console.debug('[RS] btn:prev_click', { renderCurrentIndex: currentIndex, pbCurrentIndex: pb.currentIndex, sentencesLength: sentences.length, pbStatus: pb.status }); pb.skipPrev(); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ChevronLeft size={24} /></button>
                    <button
                        onClick={togglePlay}
                    ...
                    <button onClick={() => { console.debug('[RS] btn:next_click', { renderCurrentIndex: currentIndex, pbCurrentIndex: pb.currentIndex, sentencesLength: sentences.length, pbStatus: pb.status }); pb.skipNext(); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><SkipForward size={24} /></button>
