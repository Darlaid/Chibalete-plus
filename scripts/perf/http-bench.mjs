...
/**
 * Cadencia de emisión en bucle ABIERTO, en ms. Con 0 el cliente es de bucle
 * cerrado: emite la siguiente petición en cuanto llega la anterior, de modo que
 * un servidor más rápido recibe MÁS carga. Eso confunde "código más rápido" con
 * "más peticiones por segundo" y penaliza al brazo eficiente en las rutas que
 * no optimiza. Con un valor > 0 ambos brazos ven el MISMO ritmo de llegada.
 */
const PACING_MS   = Number(args.pacingMs ?? 0);
    ...
    const doWave = async (waveIndex, keepBodyFor) => {
        ...
        if (CONCURRENCY <= 1) {
            for (const r of order) {
                ...
                if (PACING_MS > 0) {
                    // Espera hasta completar la ranura: el ritmo de llegada lo
                    // fija el reloj, no la velocidad del servidor.
                    const rest = PACING_MS - (performance.now() - slotStart);
                    if (rest > 0) await new Promise((r2) => setTimeout(r2, rest));
                }
            }
        } else {
